...
class Robot:
	...
	def _astar(self, start: Tuple[int, int], goal: Tuple[int, int], avoid_next_pos: Optional[Tuple[int, int]] = None) -> List[Tuple[int, int]]:
		...
		def get_neighbors(pos: Tuple[int, int], time: int) -> List[Tuple[int, int]]:
			...
			for dx, dy in dirs:
				...
				if (new_time == 1 and avoid_next_pos and (new_x, new_y) == avoid_next_pos):
					continue

				if 0 <= new_x < len(self.warehouse.map_) and 0 <= new_y < len(self.warehouse.map_[0]):

					is_obstacle = self.warehouse.map_[new_x][new_y] not in (0, 1)
					is_goal = (new_x, new_y) == goal

					if not is_obstacle or is_goal:
						# check if it WILL be occupied
						if not self.warehouse.is_occupied_at_time(new_x, new_y, new_time, exclude_robot_id=self.robot_id):
		...
		parent = {}  # (row, col, time) -> (prev_row, prev_col, prev_time)
	...
	def update(self, should_move: bool):
		# fix robot getting stuck
		if self.current_order and not self.path:
		...
		if self.state == "moving" and self.path and should_move:
			...
			if not self.warehouse.is_position_occupied(next_pos[0], next_pos[1], exclude_robot=self.robot_id):
				# print(f"robot {self.robot_id} path is not blocked")
				...
				# path is blocked
				print(f"Robot {self.robot_id} blocked at {(next_pos[0], next_pos[1])}. replanning...")
				...
				if self.current_order:
					self.goto(self.current_order.shelf.position, avoid_next_pos=(next_pos[0], next_pos[1]))
